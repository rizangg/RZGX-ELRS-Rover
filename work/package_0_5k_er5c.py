...
def package(reference_path):
    ...
    if payload.count(EXPECTED_VERSION) != 1:
        raise RuntimeError("Bare build does not contain exactly one 4.0.1.5K marker")
    ...
    if data.count(EXPECTED_VERSION) != 1:
        raise RuntimeError("Packaged image lost or duplicated the 4.0.1.5K marker")
...
def main():
    parser = argparse.ArgumentParser(description="Package and validate RZGX Rover 0.5K for RadioMaster ER5C V2")
    parser.add_argument(
        "--er5-reference",
        required=True,
        type=Path,
